...
namespace DataLayer
{
    public class AdminDAL
    {
        ...
        public List<string> GetTableNameDAL()
        {
            ...
            using (var conn = DBProviderDAL.Connect())
            {
                using (var cmd = new SqlCommand(query, conn))
                {
                    ...
                    {
                        while (reader.Read()) tableNames.Add(reader.GetString(0)); // Lấy tên cột khóa chính
                    }
                }
            }
        ...
        public List<string> GetPrimaryKeysDAL(string tableName)
        {
            ...
            var query = "SELECT kcu.COLUMN_NAME" +
            ...
            using (var conn = DBProviderDAL.Connect())
            {
                using (var cmd = new SqlCommand(query, conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read()) primaryKeys.Add(reader.GetString(0)); // Lấy tên cột khóa chính
                    }
                }
            }
        ...
        public List<string> GetForiegnKeysDAL(string tableName)
        {
            ...
            using (var conn = DBProviderDAL.Connect())
            {
                using (var cmd = new SqlCommand(query, conn))
                {
                    ...
                    {
                        // Đọc kết quả trả về
                        while (reader.Read()) foreignKeys.Add(reader.GetString(0)); // Lấy tên cột khóa ngoại  
                    }
                }
            }
        ...
        public Dictionary<string, string> GetAllForeignKeysAndTables(string sourceTable)
        {
            ...
            {
                var refCol = row["ReferencedColumn"].ToString();
                var refTable = row["ReferencedTable"].ToString();
        ...
        {
            var foreignKeyValues = new List<string>();
            var query = $"SELECT DISTINCT [{fkColumn}] FROM [{sourceTable}] WHERE [{fkColumn}] IS NOT NULL";


            var dt = DBProviderDAL.GetDataTable(query);

            foreach (DataRow row in dt.Rows)
            {
                var value = row[fkColumn].ToString().Trim();
        ...
        public Dictionary<string, List<string>> GetForeignKeyValuesWithReferencedTablesDAL(string sourceTable)
        {
            ...
            var foreignKeysAndTables = GetAllForeignKeysAndTables(sourceTable);
            ...
            {
                var fkColumn = fk.Key; // Cột khóa ngoại
                var referencedTable = fk.Value; // Bảng tham chiếu
                // 4. Lấy giá trị của cột khóa ngoại từ bảng nguồn
                var foreignKeyValues = GetForeignKeyValues(fkColumn, referencedTable);
        ...
        {
            var query = "SELECT * FROM MonMoDangKy";
        ...
        {
            var query = "SELECT * FROM GiangVien ";
            return DBProviderDAL.GetDataTable(query);
        }
        ...
        {
            var query = "SELECT * FROM LopMonHoc";
            return DBProviderDAL.GetDataTable(query);
        }
        ...
        {
            var query = "\t\tSELECT GiangVien.MSGV,ThoiKhoaBieu.*,LopMonHoc.Ma_Hoc_Ky FROM ThoiKhoaBieu,GiangVien,LopMonHoc\n   " +
                "             WHERE GiangVien.MSGV=LopMonHoc.MSGV AND ThoiKhoaBieu.Ma_Lop_Mon_Hoc=LopMonHoc.Ma_Lop_Mon_Hoc";
            return DBProviderDAL.GetDataTable(query);
        }
        ...
        {
            var query = "INSERT INTO MonMoDangKy (Ma_Lop_Mon_Hoc,Ma_Hoc_Ky)  " +
        ...
        {
            var query = "INSERT INTO SinhVien " +
            ...
            SqlParameter[] parameters =
            {
                ...
                new SqlParameter("@Anh", SqlDbType.VarBinary)
                    { Value = sv.Anh ?? (object)DBNull.Value } // Chuyển ảnh thành DBNull nếu null
            };
        ...
        {
            var query = "INSERT INTO GiangVien (MSGV, Ho_Ten, Gioi_Tinh, Ngay_Sinh, Dia_Chi, Email, Ma_Khoa) " +
        ...
        {
            var query = "INSERT INTO MonHoc (Ma_Mon_Hoc, Ten_Mon_Hoc, So_Tin_Chi,He_So_QT) " +
        ...
        {
            var query =
                "INSERT INTO LopMonHoc (Ma_Lop_Mon_Hoc, Ma_Mon_Hoc, MSGV, Ma_Hoc_Ky, Ma_Khoa, So_Luong_Dang_Ky_Toi_Da) " +
            ...
            SqlParameter[] parameters =
            {
                ...
                new SqlParameter("@MaHocKi", lopMonHoc.MaHocKy),
        ...
        {
            var query = "INSERT INTO Diem (MSSV, Ma_Mon_Hoc, Ma_Hoc_Ky, Diem_Qua_Trinh, Diem_Thi, Lan_Thi) " +
        ...
        {
            var query =
                "INSERT INTO ThoiKhoaBieu ( Ma_Lop_Mon_Hoc, Ngay_Hoc, Gio_Bat_Dau, Gio_Ket_Thuc, Phong_Hoc, Ngay_BD, Ngay_KT) " +
        ...
        {
            var query =
                "INSERT INTO LichThi ( Ma_Lop_Mon_Hoc, Ngay_Thi, Ma_Hoc_Ky, Gio_Bat_Dau, Gio_Ket_Thuc, Phong_Thi) " +
        ...
        {
            var query = "DELETE FROM MonMoDangKy " +
        ...
        {
            var query = "DELETE FROM SinhVien WHERE MSSV = @MSSV";
        ...
        {
            var query = "DELETE FROM GiangVien WHERE MSGV = @MSGV";
        ...
        {
            var query = "DELETE FROM MonHoc WHERE Ma_Mon_Hoc = @MaMonHoc";
        ...
        {
            var query = "DELETE FROM LopMonHoc WHERE Ma_Lop_Mon_Hoc = @MaLopMonHoc";
        ...
        {
            var query = "DELETE FROM Diem WHERE MSSV = @MSSV AND MaMonHoc = @MaMonHoc AND MaHocKy = @MaHocKy";
            SqlParameter[] parameters =
            {
        ...
        {
            var query = "DELETE FROM ThoiKhoaBieu WHERE Ma_TKB = @MaTKB";
        ...
        {
            var query = "DELETE FROM LichThi WHERE Ma_Lich_Thi = @MaLichThi";
        ...
        {
            var query = "DELETE FROM TaiKhoan WHERE ten_Dang_Nhap = @tenDangNhap";
        ...
        public bool UpdateLecturerDAL(GiangVien gv)
        {
            ...
            SqlParameter[] parameters =
            {
                ...
                new SqlParameter("@MaKhoa", gv.MaKhoa),
                new SqlParameter("@Anh", SqlDbType.VarBinary)
                    { Value = gv.Anh ?? (object)DBNull.Value } // Chuyển ảnh thành DBNull nếu null
            };
